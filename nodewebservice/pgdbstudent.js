...
function GetClient()
{
	return new Client({
		...
		// server: postgres (change db to postgress) - other fields are above on 8080
	});	
}
...
{		
	...
	try
	{
		...
		if(results.rowCount > 0)
		{
			...
			res.send({}); //or could have done res.end()s
		}
		else
		{
...
{		
	...
	try
	{
        // pragmatic patch - don't update if dont provide the detail
...
// ---- previous typed from slides - same as above but incomplete
...
// {
//     let client = GetClient();
//     console.log('Reading all the student details');
//     try {
